...
import { useUpdateStatus } from "@/lib/hooks/use-statuses";
...
export function EditStatusDialog({
    ...
    const updateStatus = useUpdateStatus(statusId, projectId);
    ...
    // Load existing status data when dialog opens
    useEffect(() => {
        if (open && project && statusId) {
            const status = project.statuses.find((s) => s.id === statusId);
            if (status) {
                // This is intentional - we want to load initial values when dialog opens
                setStatusName(status.name);
                setStatusColor(status.color);
                setStatusUnicode(status.unicode);
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Update the status column settings
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleUpdateStatus} className="space-y-4">
                    ...
                    <Button
                        type="submit"
                        className="w-full"
                        disabled={updateStatus.isPending || !statusName.trim()}
                    >
